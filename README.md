...
- `glueops_alerts.opsgenie_apikey` -- These are unique per cluster deployment. You can get these values from the terraform you use to get loki s3, vault backup, and certmanager creds.
- `glueops_backups` -- Configuration for backups taken by the GlueOps Platform.
  - `vault` -- Credentials and configuration for backing up vault, values are generated by [terraform-module-cloud-multy-prerequisites](https://github.com/GlueOps/terraform-module-cloud-multy-prerequisites) and are specific to this backup process, due to narrowly scoped permissions
- `externalDNS` -- Values are specific to `externalDNS` and generated by [terraform-module-cloud-multy-prerequisites](https://github.com/GlueOps/terraform-module-cloud-multy-prerequisites)
- `certManager` --
  - `zerossl_eab_kid` and `zerossl_eab_hmac_key` can be obtained for free with an account under zerossl.com.  To retrieve these values, log in to the relevant zerossl account for your zone and navigate to the [developer page](https://app.zerossl.com/developer). **_Note: For development, use a personal ZeroSSL account, and not a shared account - this avoids unintended consequences. Each customer will has a unique ZeroSSL account as well._**
  - `aws_accessKey`, `aws_secretKey`, `aws_region` -- Values are specific to `certManager`, due to narrowly scoped permissions, and are generated by [terraform-module-cloud-multy-prerequisites](https://github.com/GlueOps/terraform-module-cloud-multy-prerequisites).
...
- `loki` -- Values are specific to `loki` and generated by [terraform-module-cloud-multy-prerequisites](https://github.com/GlueOps/terraform-module-cloud-multy-prerequisites) 
